...
class QualitaetKategorie(models.Model):
	name = models.CharField(max_length=255,verbose_name='Qualität')

	def __unicode__(self):
		return self.name

	class Meta:
		verbose_name_plural = 'Qualität'
		verbose_name = 'Qualität'
        
class QualitaetKatForm(forms.ModelForm):

    #beschreibung = forms.CharField( widget=forms.Textarea )
    class Meta:
        model = QualitaetKategorie
...
class Wein(BaseProduct):
	...
	saale = models.BooleanField(verbose_name='Saale-Unstrut', default=True)
	...
	qualitaet = models.ForeignKey('QualitaetKategorie',blank=True, null=True, verbose_name='Qualität')
